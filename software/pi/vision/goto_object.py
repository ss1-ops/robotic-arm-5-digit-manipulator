...
def fk_base_ee(q5):
    # Returns the 4x4 homogeneous transform in the *internal chain frame*
    # (use this for transforming points that are defined in the model/chain
    # frame, such as CAM_T offsets and depth points from reproject).
    # The final base-frame position is then converted to user frame with to_user
    # (or mp.chain_to_user) for targets and logging.
    return mp.forward_kinematics_matrix(q5) if hasattr(mp, 'forward_kinematics_matrix') else mp.MOVEO_CHAIN.forward_kinematics([0.0] + list(q5) + [0.0])


# Backward-compat shim. New code should use mp.forward_kinematics or the
# chain_to_user / user_to_chain helpers from kinematics / moveo_publisher.
def to_user(p):
...
class GotoObject(Node):
    def __init__(self):
        ...
        self.get_logger().info(
            ...
            f"Object in view + Send All Joints.  !! E-STOP ready.")
    ...
    def _load_cal(self, path):
        ...
        sgbm = cv2.StereoSGBM_create(
            minDisparity=0, numDisparities=160, blockSize=7,  # increased; close objects need ~100+ px disp at 0.5 scale (22cm object needs ~114px)
    ...
    # ── multi-frame depth after centering (matches depth_test_GUI robustness) ──
    def _measure_depth(self):
        """Capture N synced L/R pairs and return median metric depth (m) at the
        colour blob centroid, plus the normalised camera-plane (x,y) offset
        (from the last good centroid)."""
        samples = []
        last_cxy = None
        t0 = time.time()
        while time.time() - t0 < 8.0 and len(samples) < max(1, self.n_samples):
            with self._lock:
                l = self._left; r = self._right; cxy = self._centroid_px
            if l and r and cxy and abs(l[0]-r[0]) < 5_000_000:
                z_m = self._depth_one_pair(l[1], r[1], cxy)
                if z_m is not None:
                    samples.append(z_m)
                    last_cxy = cxy
                # consume to get fresh
                with self._lock:
                    self._left = self._right = None
            time.sleep(0.03)
        if not samples or last_cxy is None:
            return None, None, None
        z_m = float(np.median(samples))
        # use the last cxy for the ray direction (view is stable after centering)
        x_n = (last_cxy[0] - self._cal["cx"]) / self._cal["fx"]
        y_n = (last_cxy[1] - self._cal["cy"]) / self._cal["fy"]
        return z_m, x_n, y_n

    def _depth_one_pair(self, left_bgr, right_bgr, cxy):
        """Compute depth (m) for one full-res pair at the given full-res cxy."""
    ...
    def _run(self):
        ...
        # ── Phase 2: multi-frame depth (matches depth_test_GUI) ──
        self.get_logger().info(f"computing depth (median over up to {self.n_samples} frames)...")
        ...
        # We use the raw IK solution (the one that actually reaches the target)
        # and only apply the j5 camera-aim correction on top.
        # This keeps the goto flow using the same IK result the GUI button uses.

        # Compute j5 so the camera aims at the object at the IK pose.
        ...
        # For debugging: what is the actual EE position in user frame for the final sent joints?
        # This uses the universal forward_kinematics (user frame) from the same kinematics.py.
        actual_user = mp.forward_kinematics(ik_aimed)
        self.get_logger().info(
            f"actual final EE (user frame FK of sent joints): x={actual_user[0]:.3f} y={actual_user[1]:.3f} z={actual_user[2]:.3f} m")
