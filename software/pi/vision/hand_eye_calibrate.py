...
cv2.calibrateHandEye, writes hand_eye.yaml, and prints a consistency residual.
  ...
  (j4 is now calibrated, so you can vary it if desired.)
...
class HandEyeNode(Node):
    def __init__(self):
        ...
        self.j4_max = p("j4_max_rad", 3.14).value          # now that j4 is calibrated we no longer force it near zero
    ...
    def image_cb(self, msg):
        ...
        if abs(self._joints[3]) > self.j4_max:
            self.get_logger().warn(
                f"[{n}/{self.count}] wrist roll j4={self._joints[3]:+.2f}  (j4 now calibrated) "
